...
std::vector<uint32_t> toIntegers(std::span<const char> batterBank) {
  return batterBank | std::views::transform([](const char val) {
...
uint32_t findMaxJoltage(const std::vector<uint32_t> &batteryBank) {
  ...
  for (const auto &[index, batPower1] : std::views::zip(
           std::views::iota(size_t{1}, numberOfBatteries + 1u), batteryBank)) {
